...
例えば、外部サイトからのリンクをクリックして「ページを表示する（GET）」時には Cookie を送りますが、外部サイトから勝手に「データを書き換える（POST）」時には送りません。これにより、**「ログイン状態を維持したままリンク移動はできるが、勝手に掲示板に書き込まれたりはしない」** という状態を実現できます。

  - ※他の方法（例：GETメソッド以外のリクエスト※や`fetch`関数などを用いたJavaScriptから送信されるリクエスト）を使ったクロスサイトからのリクエストには**Cookieを付与しないのでCSRF対策**となる
    - ※状態を変化させる可能性のあるメソッド（PUT, DELETEなど）や、`<iframe>`内での読み込み、画像タグ (`<img>`) によるリクエストでも Cookie は付与されません。もし Lax が「すべての GET」で Cookie を送ってしまったら、攻撃者は画像タグ`例：<img src="https://bank.com/transfer?amount=1000">`を自分のサイトに埋め込むだけで、ユーザーが画像を見た瞬間にリクエストが飛んでしまう（CSRF）リスクが残るため

「URLが変わるような画面遷移」に限定することで、ユーザーが自発的にそのサイトへ移動しようとしている意思を尊重しつつ、裏側での勝手なリクエストを防いでいます。
