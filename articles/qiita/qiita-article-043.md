...
※ 後述の[【救済措置】git reset --hard で削除した内容を復元する](#救済措置git-reset---hard-で削除した内容を復元する)を取り消せます。

::: note info

### `git reset`コマンドの各種オプションの違いについて

- `git reset --soft`
コミット（ローカルリポジトリとの接続および編集履歴＝HEADが指すコミットを変更）を取り消すが、ステージング（add した状態）とワーキングツリー（ローカル環境でのファイル編集・操作）は残す。

- `git reset --mixed`
デフォルト（= git reset）。アンステージング（add の取り消し=ステージング環境のリセット）するが、ワーキングツリー（ローカル環境でのファイル編集・操作）は残す。

- `git reset --hard`
ステージングおよびワーキングツリーの全てをリセット（削除）する。つまり、なにもファイルが編集・操作されていない（当該ローカル環境の最新）状態にまで戻す。
...
## センシティブな情報を誤って push してしまった場合

`.env` ファイルや APIキーなど、公開すべきでない情報を誤って push してしまうケースがあります。

「`git reset --hard` して force push すれば消えるはず」と思いがちですが、**それだけでは完全な削除にはなりません**。
GitHub 側にはキャッシュや PR 参照など、Git 履歴とは別の場所にデータが残り続けることがあるためです。[※詳細はこちら](#force-push-しても消えないもの)。

### 対応の基本的な考え方

まず「漏れた情報の種類」で対応が変わります。

| 漏れた情報の種類 | 対応方針 |
|---|---|
| APIキー・トークン・パスワード | **まず無効化（revoke / rotate）** を優先。履歴削除はその後 |
| 秘密鍵・証明書 | 無効化 ＋ 履歴削除の両方が必要 |
| 個人情報・顧客情報 | 履歴削除 ＋ GitHub サポートへの依頼まで必要 |

::: note info
ローテーション（初期化や非活性化）できる認証情報であれば、**無効化さえ済ませれば、履歴に残っていても実害がない状態**にできます。焦って履歴を操作する前に、まず無効化を先に行いましょう。
:::

---

### 履歴から削除する方法： `git-filter-repo`
`.env` などのファイルを Git 履歴ごと消したい場合、`git reset --hard` より **`git-filter-repo`** の使用が現在の標準的な方法です（[GitHub Docs](https://docs.github.com/ja/authentication/keeping-your-account-and-data-secure/removing-sensitive-data-from-a-repository) でも案内されています）。

```bash
# 事前インストール（Homebrewの場合）
...
特定の文字列（APIキーの値など）だけを置換したい場合：

```bash
# 置換ルールファイルを用意
echo "literal:実際のAPIキーの値" > ../secrets.txt
...
::: note warn
`git push --force --mirror` は全ブランチ・タグを強制上書きします。Branch Protection が有効な場合は一時的に解除が必要になることがあります。他のメンバーへの影響も考慮したうえで実行してください。
:::

### force push しても消えないもの

force push 後も、以下の場所にはデータが**残り続ける場合があります**。

- GitHub のキャッシュ（SHA-1 経由のキャッシュビュー）
- PR の参照（`refs/pull/`）
- fork したリポジトリ
- CI ログや artifact

これらを完全に消すには **GitHub サポートへの依頼**が必要です。  
（GitHub Enterprise Server の場合は自社の GHES 管理者が対応します）

### 再汚染に注意

履歴を書き換えると commit ハッシュが変わります。  
**古いクローンを持っている開発者が `git pull` → `git push` すると、書き換え前の履歴が再度混入してしまう**ことがあります。

チームで作業している場合は、履歴書き換え後に必ず以下を周知してください。
...
どうしても既存のクローンを使い続ける場合は、`merge` ではなく `rebase` で取り込む必要があります。

---

### 予防策：push 前に検知する

事後対応は手間がかかるため、**そもそも push させない仕組み**を入れておくのが理想です。

**.gitignore に最低限追加しておくもの**
...
**Gitleaks を使った pre-commit フック（ローカルでの検知）**
...
これで commit 時に機密情報が含まれていれば自動でブロックしてくれます。  
ただし `SKIP=gitleaks git commit ...` のようにスキップできてしまうため、あくまで「手元での予防」の位置づけです。

GitHub の設定で **Push Protection**（Settings → Code security → Secret scanning → Push protection）を有効にすると、リモートへの push 時にもブロックがかかります。
