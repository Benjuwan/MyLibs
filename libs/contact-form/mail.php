...
$site_top = "https://ドメイン/サブドメイン/特定dir";
...
$Referer_check_domain = "ドメイン/サブドメイン/特定dir";
...
$require = array('お名前','メールアドレス','電話番号','お問い合わせの種類','プライバシーポリシー','送信確認');
...
//確認画面無しの場合の表示、指定のページに移動する設定の場合、エラーチェックで問題が無ければ指定ページヘリダイレクト
	...
	if($empty_flag == 1){ ?>
<div style="text-align: center;"><h4>入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</h4><div style="color:red"><?php echo $errm; ?></div><br /><br /><input type="button" class="resetBtn" value=" 前画面に戻る " onClick="history.back()"></div>
<?php 
...
function refererCheck($Referer_check,$Referer_check_domain){
	if($Referer_check == 1 && !empty($Referer_check_domain)){
		if(strpos($_SERVER['HTTP_REFERER'],$Referer_check_domain) === false){
			return exit('<p style="text-align: center;">リファラチェックエラー。フォームページのドメインとこのファイルのドメインが一致しません</p>');
		}
	}
}
function copyright(){
