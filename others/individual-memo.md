...
`MCP`を非エンジニア向けに咀嚼すると以下となる。
- 生成AI（各種LLM）が外部サービス（例：`GitHub`や`PostgreSQL`（外部のデータベース）、社内ファイルシステム）と連携できる規格
- 連携することで生成AIが連携先情報を活用して自律的に働いてくれる機能（＝AIが自動・自律的に他のソフトウェアやサービスを操作できる仕組み）<br><br>
実際に用いるには、後述する`MCPクライアント`と`MCPサーバー`が必要となる。

##### MCPクライアント
`MCPクライアント`とは、 **人間とAIモデルとの仲介ポイント（操作指示を行うための窓口）** を指す。プログラミング領域で言うと、`Claude Desktop`,`Cline`,`Cousor`のようなAIコードエージェンシー（コーディング・プログラミングを補佐または主導してくれる機能）を指す。<br>
これらを`MCPサーバー`（連携先の外部サービス）と接続することで、先に説明したような機能拡張を実現できる。

##### MCPサーバー
`GitHub`,`PostgreSQL`,ファイルシステムなど **生成AI（各種LLM）と連携する（機能を備えた）接続先との仲介ポイント** を指す。例えば、**OSとアプリケーションの間にあるミドルウェアのようなもの**とイメージできる。

##### 具体例な使用方法
先の、`MCPクライアント`と`MCPサーバー`を接続（連携）することで利用可能。<br>
原則として、`MCPクライアント`に使用（＝`MCP`として動かす）するための`生成AIのAPIキー`が必要となる。<br>
このAPIキーは、ベース料金およびトークンの使用量に応じた従量課金制なのが一般的。
