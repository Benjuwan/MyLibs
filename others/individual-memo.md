...
`GitHub Copilot`や`Windsurf`,`Cursor`などがある。<br>
情報収集する中での感覚としては現時点（2025/05）において、**個人やチームの環境/状態/条件などによりけりではあるものの**、「`Cursor` > `GitHub Copilot`（または`Cline`） > `Windsurf`という印象」を受けた。
