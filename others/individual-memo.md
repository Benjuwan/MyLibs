...
1. 参照情報ソース（用語集やテンプレート、ドキュメントなど）を、`Markdown`ファイル（.md）で作成する。`.pdf`, `.csv`, `.xlsx`, `.docx`などを`.md`に変換する作業は LLM にしてもらう。
  - いきなり全てをAIに教えるよりも、まずは一つの知識エリア、一つのドキュメントテンプレートから試してみるのがベター。そして徐々に範囲を広げていくのが現実的だそう。<br><br>

2. 作成した`Markdown`ファイルを、プロジェクトルートの`.cursor/docs/`ディレクトリに格納する。これにより、AIチャットで`@docs <ディレクトリパス込みのファイル名>`を使って`Cursor`にこれらの情報を参照してもらうことが可能。<br><br>
