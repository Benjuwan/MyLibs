...
### AIとどう付き合っていくか
- 現状<br>
AIに関する形式知と暗黙知を蓄積する戦略（スタンス）を採っていきたい。
  - 理由<br>
  ...
  これまで大切だった課題解決力など(※人と人との**コミュニケーションが必要な部分以外**の)実務面はAIが担っていくと考えられるので、**人はものごと及びそれへのアプローチを考える役割が強く**なりそう。

- 今後必要と考えている「問題把握力と問題想起力」とは<br>
  - **問題把握力**<br>どこにどんな問題があるかのアタリを付ける（アプローチ範囲を設定する）
  - **問題想起力**<br>何がペインまたはボトルネックかを考える
つまり、課題解決における「方針や仮説出し、アプローチ方法の選定など上流工程部分」に関わってくるようなもの。<br><br>
...
`MCP`を通じて、**各AIモデルが外部ツール（`GitHub`など）やデータソース（`PostgreSQL`など）と安全かつシームレスに連携できる**ようになる。ちなみに、`Cline`は`MCP`の一種。
