...
> [!NOTE]
> カバレッジレポートには**数値が高いから品質の高いテストであるという相関性はない**ので注意。「網羅率80%以上ならCIはパスする」といった定量的判断されることは多いものの、あくまで**テスト実行時に通過したかどうかの判定に留まる**だけでバグがないことを保証する性質は持っていない。<br>
> とはいえ、カバレッジの低いファイルはテストが不足している証拠で、追加テストを書くかどうかの検討材料になるし、デッドコードの発見にも寄与する。<br>
...
#### `Branch`（分岐 網羅率）
テスト対象ファイルに含まれる「全ての条件分岐」が少なくとも1回通過したかを表す。`if`文や`case`文、三項演算子の分岐が対象となる。<br>
...
このコマンドを実行するとブラウザが立ち上がってグラフィカルなカバレッジレポートが表示される。ページ内容はカバレッジ全体の概要と、テスト対象ファイルのカバレッジ一覧となっている。<br>
テスト対象ファイルのカバレッジ一覧では、緑色のセルはテストが十分書かれていることを示しており、黄色と赤色のセルは不足していることを示している。

#### `{d}x`： 該当行がテストで通過した回数
カバレッジレポートでは、テスト対象ファイルの実装内部における一行一行に対してテストが通過しているかどうかの状況確認を行える。<br>
その確認指標となるのが「テスト対象コードの行数横に表示される`{d}x`という数値」で、例えば`6x`は該当行が6回通過していることを指す。<br>
**呼び出しを通過している**ことと**分岐を通過している**ことの2点を意識してテストを書くのがカバレッジ向上のコツとなる。<br>
...
### カスタムレポーター
`jest.config.js/ts`ファイルに設定を追記することで、テスト結果のグラフ表示や時間がかかっているテストの調査、ソート機能など便利な機能を持ったカスタムレポートを作成できるようになる。<br>
