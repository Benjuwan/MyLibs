...
#### `toBeTruthy`
`true`そのものだけでなく、`true`と同等に評価されるtruthyな値（`1`, `"1"`など）全般に一致する。

#### `toBeFalsy`
`false`そのものだけでなく、`false`と同等に評価されるfalsyな値（`0`, `""`など）全般に一致する。  
...
#### `toBeCloseTo`
JavaScriptでは**10進数の小数を2進数に変換する時に小数計算に誤差が生じてしまう**。  
少数の正確な計算ができるライブラリを使用せずに小数計算を検証する場合に`toBeCloseTo`マッチャーを利用する。  
...
- 失敗処理（`reject`）の場合  
`try{} catch{}`文を使用する方法。`timeout(50)`が`reject`した際の例外を`try`ブロック内で`await`しているため、その`reject`は`try/catch`で捕捉される（unhandledにはならない）。捕捉したエラーを`catch`ブロック内でアサーションによって検証するという方法。
```ts
test("指定時間待つと、経過時間をもって reject される", async () => {
  // アサーションが一度実行されることを期待する
  // これがないと、catchブロックが実行されなかった（＝失敗が検知されなかった）場合でもテストが誤って成功扱いになってしまう
  expect.assertions(1);
...
> [!NOTE]
> - `expect.assertions(実行される回数の期待値)`：  
> アサーションが実行されることそのものを検証し、引数には実行される回数の期待値を設定する  
> **`expect.assertions`はテストの終了を防ぐ機能ではなく、アサーションが実行されなかったことを検知する仕組み**である点に注意。非同期処理の完了（Promiseの解決）を待つのはあくまで`await`/`return`の役割であり、`expect.assertions`はその上で「意図した回数のアサーションが実際に実行されたか」を保証するために併用する。
...
#### 非同期処理をテストする際の注意事項
以下のテストコードでは`return`文がないため、`wait`関数の`Promise`が解決する前にテストが終了してしまう。  
つまり、**アサーションが実行されないままテストが完了してしまうため、意図した検証が行われない**ことになる。  
...
### カスタムマッチャー（`jest-dom`）
UIコンポーネントのテストでも Jest のアサーションやマッチャーを利用できるものの、DOMの状態を検証するには Jest 標準だけでは不十分な場合がある。  
