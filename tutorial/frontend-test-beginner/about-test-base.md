...
- `テスト対象モジュール名.ts` -> `テスト対象モジュール名.test.ts`
    - `.spec.ts`: ユニットテスト用と説明されることもある（各モジュールや関数の仕様＝specificationをテストする、というニュアンス）。
    - `.test.ts`: 統合テスト用と説明されることもある（複数のモジュールやサービスが正しく連携して動作するかをテストする、というニュアンス）。

※これら拡張子には標準的な使い分けが存在するわけではない。どちらの拡張子を使用するかは、プロジェクトやチームの慣習、または個人の好みに依存している。書籍では`.test.ts`で統一されている。

### テストファイルの配置パターン
- `__tests__`ディレクトリを用意して、その中に配置する
...
```ts
// ※説明用の断片コードのため第二引数（テスト関数）を省略している。
// このまま実行するとJestが"Missing second argument"エラーを投げるため、実際に動かす場合は次項の第二引数（テスト関数）を含めた形で記述すること。
test("1 + 2 = 3");
```
...
> [!NOTE]
> `test関数`は先頭に`x`を付与（`xtest`と記述）すると**当該テストを実行しない（スキップする）ことができる**。
...
> [!NOTE]
> 上記の`RangeError`はJavaScript組み込みの`RangeError`（グローバルオブジェクト）と同名であり、スコープ内で組み込みの`RangeError`をシャドーイング（隠蔽）してしまう点に注意。実プロジェクトでは`CustomRangeError`など、組み込みエラーと衝突しない名前を検討すること。
